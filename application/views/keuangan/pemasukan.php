                            ...
                            <table id="multi-filter-select" class="display table table-striped table-hover">
                                ...
                                <tbody>
                                    ...
                                    <?php foreach ($dataTab as $row) : ?>
                                        <tr>
                                            <td><?= $i++ ?></td>
                                            <td><?= tanggal_indonesia(date('Y-m-d', $row->tanggal_masuk)); ?></td>
                                            <td><?= number_format($row->jumlah, 0, ',', '.'); ?></td>
                                            <td><?= ucfirst($row->sumber) ?></td>
                                            <td><?= $row->keterangan ?></td>
                                            <td><?= $row->asrama ?></td>
                                            <td class="text-center">
                                                <a href="<?= base_url('keuangan/ubahPemasukan/' . $row->id) ?>">
                                                    <span class="badge bg-warning"><i class="bi bi-pencil-square me-1"></i> Ubah</span>
                                                </a>
                                                <a href="<?= base_url('keuangan/hapusPemasukan/') . $row->id ?>" onclick="return confirm('Apakah anda yakin')">
