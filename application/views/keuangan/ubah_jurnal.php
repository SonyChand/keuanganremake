                    ...
                    <div class="card-body">
                        ...
                        <form class="row g-3" method="post">
                            ...
                            <div class="col-md-6">
                                ...
                                <select id="pengguna" class="form-select" name="pengguna" required>
                                    ...
                                    <?php foreach ($dataMod as $user) : ?>
                                        <option value="<?= $user->nama ?>" <?= $jurnal->nama_pengguna == $user->nama ? 'selected' : '' ?>>
                                            <?= $user->nama ?>
                                        </option>
                                    <?php endforeach; ?>
                                </select>
                            ...
                            <div class="col-md-6">
                                ...
                                <input type="number" class="form-control" name="debet" id="debet" value="<?= $jurnal->debet ?>" step="0.01" required>
                                <?= form_error('debet', '<small class="text-danger">', '</small>'); ?>
                            </div>
                            <div class="col-md-6">
                                <label for="kredit" class="form-label">Kredit</label>
                                <input type="number" class="form-control" name="kredit" id="kredit" value="<?= $jurnal->kredit ?>" step="0.01" required>
                                <?= form_error('kredit', '<small class="text-danger">', '</small>'); ?>
                            </div>
                            <div class="text-end mt-5">
                                <a href="<?= base_url('keuangan/jurnal_umum') ?>" class="btn btn-secondary">Kembali</a>
